document.addEventListener('DOMContentLoaded', () => {
    ...
    const currentFileNameStatusSpan = document.getElementById('current-file-name-status');
    const fileListUl = document.getElementById('file-list'); // For displaying current file name in sidebar
    ...
    btnPaste.addEventListener('click', async () => {
        try {
            ...
            // document.execCommand('insertText', false, text); // Old way, might not work in all contexts securely
            // A more robust way for modern browsers:
    ...
    function printContent() {
        // Create an iframe to print only the textarea content
    ...
    function execCmd(command, value = null) {
        try {
            // Note: document.execCommand is considered deprecated for some commands.
            // For copy/cut/paste, Clipboard API is preferred but more complex for a simple demo.
            if (!document.execCommand(command, false, value)) {
                // Fallback or error message if command is not supported or failed
                // For paste, navigator.clipboard.readText() is better (as used above)
                if (command === 'paste') {
                    alert('Pasting via execCommand failed. Try Ctrl+V or use the dedicated paste button.');
                } else {
                    // console.warn(`Command '${command}' might not be fully supported or failed.`);
                }
            }
            notepadArea.focus(); // Keep focus on textarea
            updateStatus(); // Some commands might change content
        } catch (e) {
    ...
    function updateFileNameDisplay(fileName) {
        ...
        // Update sidebar file list (simple version, just replaces content)
